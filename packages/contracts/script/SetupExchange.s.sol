...
import { ExchangeFee } from "../src/codegen/tables/ExchangeFee.sol";
...
contract SetupExchange is Script {
  function run(address worldAddress) external {
    ...
    for (uint i = 0; i < bazaarConfig.items.length; i++) {
      ...
      require(
        ChipTable.getChipAddress(chestEntityId) == 0x0a8E974cfd0fa2933891Ea20e6AD5c83f9B304cc,
        "Invalid chip address"
      );
      ChipTable.setChipAddress(chestEntityId, chipAddress);
      ...
      ExchangeFee.set(chestEntityId, objectTypeId, uint256(1));
