...
contract Chip is IChip {
  ...
  function setDisplayDescription(bytes32 entityId, string memory name) public {
    require(
      ChipAttachment.getAttacher(entityId) == msg.sender,
      "Only the attacher can set the force field description"
    );
    setForceFieldDescription(entityId, name);
  }
