...
contract Chip is IChip {
  ...
  function onTransfer(
    ...
    if (isDeposit) {
      if (transferObjectTypeId == ChipObjectID && numToTransfer == 1 && !MintedNFT.getMinted(player)) {
        ...
        setNotification(player, unicode"You've earned the Settlers Union Bank Pass!");
