  ...
  function initChip() internal {
    setChipMetadata(
      ChipMetadataData({
        ...
        name: "Buy & Sell Blocks via Tokens",
        description: "Players receive tokens for placing blocks in your chest, let players take blocks from your chest by sending tokens. Token price is 1-1 with supply in network."
      })
    );
  }

  function setDisplayName(bytes32 chestEntityId, string memory name) public {
    require(ChipAttachment.getAttacher(chestEntityId) == msg.sender, "Only the attacher can set the chest name");
    setChestName(chestEntityId, name);
  }
