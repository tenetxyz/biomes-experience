...
contract Chip is IChestChip, LinearVRGDA {
  ...
  function onTransfer(
    ...
    {
      // Update price
      int256 timeSinceLastPurchase = toDaysWadUnsafe(block.timestamp - Exchange.getLastPurchaseTime(objectTypeId));
      uint256 vrgdaPrice = getVRGDAPrice(timeSinceLastPurchase, Exchange.getSold(objectTypeId));
      // Adjust price by rarity factor
      Exchange.setPrice(objectTypeId, (vrgdaPrice * Exchange.getRarity(objectTypeId)) / 5);
    }
    uint256 currentlySold = Exchange.getSold(objectTypeId);

    if (isDeposit) {
      Exchange.setSold(objectTypeId, numToTransfer < currentlySold ? currentlySold - numToTransfer : 0);
      ...
      Exchange.setSold(objectTypeId, currentlySold + numToTransfer);
      Exchange.setLastPurchaseTime(objectTypeId, block.timestamp);
